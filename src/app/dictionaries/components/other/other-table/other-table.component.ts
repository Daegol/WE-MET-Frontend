...
import { OtherFilter } from '@we-met-app/dictionaries/interfaces/other-filter.interface';
...
export class OtherTableComponent implements AfterViewInit, OnInit, OnChanges {
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatTable) table: MatTable<SubCategoryDto>;

  @Input() others: SubCategoryDto[];
  @Input() globalFilterValue: string;
  @Input() selectedOthers: SubCategoryDto[];
  ...
  @Output() selectedOthersEvent = new EventEmitter<SubCategoryDto[]>();
  @Output() rowSelectedEvent = new EventEmitter<SubCategoryDto>();

  readonly empty = "EMPTY";
  private filteredValues: OtherFilter = { global: '' }
  ...
  constructor() {
    ...
    this.selectedOthers?.forEach(other => this.selection.toggle(other));
  }
  ...
  private applySelectionChanges(): void {
    ...
    this.selectedOthers.forEach(x => this.selection.toggle(x))
  }

  private initDataSource(): void {
    this.dataSource = new MatTableDataSource<SubCategoryDto>(this.others);
  ...
  filterData(data?: SubCategoryDto, filter?: string): boolean {
    let searchString = JSON.parse(filter) as OtherFilter;
  ...
  applyChanges(changes: SimpleChanges): void {
    ...
    this.setDataSourceData(changes.others?.currentValue);
    this.applyFilter();
  }
  ...
  toggleSelected(row?: SubCategoryDto): void {
    if (this.isSelected(row)) {
      this.selectedOthersEvent.emit(this.selection.selected.filter(item => item.id != row.id));
    } else {
      this.selection?.toggle(row);
      this.selectedOthersEvent.emit(this.selection?.selected);
    }
  }

  openNewOtherPopup(): void {
    this.openNewPopup.emit();
  }

  openEditOtherPopup(row: SubCategoryDto): void {
  ...
  openDeleteOtherPopup(row: SubCategoryDto[]): void {
    this.openDeletePopup.emit(row);
  }

  isSelected(row: SubCategoryDto): boolean {
    return this.selectedOthers.find(item => item?.id == row?.id)?.id != null;
  }
